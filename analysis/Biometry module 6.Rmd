...
chart(data = biometry, ~ factor(0) %fill=% sequences) +
  geom_bar(width = 1) + 
...
```{r}
group_by(biometry, age_rec , rapport_imc)%>.%
summarise (., n = n() )%>.%
spread(., key = "rapport_imc" , value="n")%>.%
...
```{r}
biometry %>.%
group_by(.,rapport_imc)%>.%
summarise (., n = n() )%>.%
spread(., key = "rapport_imc" , value="n")-> biometry_chi
...
```{r}
chart(data = biometry, ~factor(sequences) %fill=% genre ) +
...
```
Sur le graphique, ce graphique est trés interessant car grâce à lui on constate que la taille et le poids varie bien en fonction de l'âge.La courbe des plus âgés ne dépasse jamais la courbe des plus jeunes.

#### Répartition de l'IMC  en fonction des catégories d'age.

```{r}
chart(data = biometry, ~factor(sequences) %fill=% age_rec ) +
...
```{r}
p=ggplot(biometry, aes(x= alcool, y= IMC , color= sequences, shape= genre)) + 
...
chart(data = biometry, ~ alcool %fill=% sequences | sequences) +
  geom_histogram(data = select(biometry, -sequences), fill = "grey", bins = 25) + 
  geom_histogram(show.legend = FALSE, bins = 25, na.rm = FALSE) +
  ylab("Effectifs") +
  scale_fill_viridis_d()
chart(data = biometry, alcool ~ sequences %fill=% sequences) + geom_boxplot(show.legend = FALSE)
