...
Notre but est déterminer la répartition de l'IMC dans notre échantillonnage. Ainsi qu'essayer de déduire les causes pouvant influencer ces répartition de l'IMC.
...
> GE: attention de ne pas mélanger la description des résultats et la discussion de ces résultats. `les pourcentages correspondent bien à la zone d'échantillonage en effet la majorité des individus sont situé dans des régions développés, avec des ressources alimentaires suffisantes.` La phrase ci-dessus fait plutot partie d'une analyse des données et pas de la description de ces dernières. Nous sommes donc plutot dans une discussion des résultats. 
...
Est ce que la masse corrigée des personnes est identique dans la population selon le genre ou est-ce que l'un des genres est en moyenne plus lourd que l autre ?:
...
> GE: il est lus intéressant d'exprimer le résultats d'un test statistique en français et d'éviter  p <<<< α


##### Representation graphique des distributions:


> l'instruction filter() n'est pas assigné et ne sert donc pas à grand chose. De ce fait vous ne faites qu'afficher un tableau de données. 
...
> GE: vous en concluez dans une discussion cela ne va pas comme terme
...
On remarque sur ces moyennes que les 60-110 ont une taille plus petite mais possède un poids dans les alentours des individus adulte. Les 0-20 ont une taille un peu près identique à celle des adultes cependant leur moyenne pour le poids est plus faible. On peut comprendre que l'IMC des personne agés sera plus important que l'IMC des jeunes. Cela s'explique par le faite que les jeunes viennent de terminer leur croissance et le corps utilise des ressources pour grandir. Par la suite le corps adulte se nourrit pour survivre, il n'avait plus besoin de nourriture supplémentaire pour sa croissance.
...
Le graphique nous révèle que les personnes en sous-poids prennent une quantité moins importante de viande. Le manque de protéine animal entraine le sous-poids.Et la surconsommation de protéine animal nous indique que cela peut entrainer une augmentation de l'IMC.
...
La troisième partie de nos analyses s'attardent sur les causes pouvant influencer l'IMC. Il y a divers facteurs qui influence l'IMC, le premier mesurés dans notre étude est l'alcool.
la moyenne d'alcool pour les différents groupes ("souspoids", "normal", "surpoids","obèse") augmente en même temps que le IMC , on peut donc faire une lien entre les 2 variables. Ce lien constiste à dire que la consommation d'alcool entraine l'augmentation de l'IMC. Les individus en sous-poids prennent moins d'alcool que les individus en surpoids.
On peut également mettre en lien, l'age et l'alcool car on a constaté plus tôt que les individus jeunes sont en général plus en sous-poids que les adultes et les personnes agés. Dans notre catégorie de jeune, il y a des personnes ayant un age inférieur à 16 ans qui est la limite d'autorisation pour l'alcool, les individus plus agé sont plus confronté à l'alcool que les jeunes.
L'autre facteur mesuré est le grignotage, les moyennes du grignotage, le groupe du "souspoids" à la moyenne la plus forte est le groupe "obèse" possède la moyenne la plus faible.Cependant les différences entre les moyennes sont relativement faible et il faut faire attention aux valeurs extrèmes qui peuvent influencé la moyenne. Il faut prendre ces résultats avec des pincettes car nous avons pas déterminer le type de grignotage : fruits ou friandises. L'analyse graphique confirme nos doutes et n'apporte pas de différences significatives entre les individus. Les moyennes étaient légèrement différentes à cause des valeurs extrèmes. 
La 3ème variable pouvant être une cause de l'influence de l'IMC c'est la proportion de protéine animal ingéré.Le groupe des "souspoids" possède la moyenne la plus faible et le groupe"obèse" possède la moyenne la plus grande. On peut conclure que les protéines animales ont une influence avec la prise de poids. Le manque de protéine animal entraine le sous-poids.Et la surconsommation de protéine animal nous indique que cela peut entrainer une augmentation de l'IMC. Pour la catégorie "normal"et"surpoids"les valeurs sont assez proche. Il doit y avoir un autre facteur qui doit influencer l'IMC. Le manque de protéine animal entraine le sous-poids.Et la surconsommation de protéine animal nous indique que cela peut entrainer une augmentation de l'IMC.
Pour la 4ème ligne de données qui est le fastfood, les résultats sont moins significatifs.On remarque que la moyenne la plus faible est attribuée à la catégorie du "souspoids", cependant la moyenne la plus forte est celle du groupe normal.Dans les groupes "souspoids","surpoids" et "obèse" on a une augmentation lié à la montée du IMC. Le groupe"normal" ne suit pas notre hypothèse, les différentes raisons sont que soit les fastfood n'ont pas de lien avec la prise de point ou soit nous avons oublié de tenir compte d'une autre variable pouvant influencer sur l'IMC. Dans l'analyse graphique, le grouppe sous-poids semble bien manger moins de fastfood mais les 3 autres groupes n'ont pas de caleurs significatives. Il doit y avoir un autre facteur qui influence les 3 groupes. Surtout pour le groupe normal qui en général mange environ une même quantité de fastfood que le groupe obèse.
Le 5ème facteur d'influence est le sport, ici aussi on trouve un influence. Les individus fesant le plus de sport sont ceux avec l'IMC le plus faible.Le sport diminue doncl'IMC des individus. Cependant on peut remarqué que le groupe fesant le plus de sport est le groupe normal et non le groupe sous-poids. 
La raison de cette variation inattendu sont les autres facteurs influençant l'IMC.Le groupe "normal" fait plus de sport mais il mange également plus de fastfood. L'IMS étant influencé par tellement de facteur, certains facteurs se compense et s'équilibre pour donner un IMC dans la catégorie Normal mais pouvant aller dans les extrêmes comme une consommation importante d'aliments augmentant le poids et beaucoup d'heures de sport diminuant le poids.
La 6ème valeur mesurée est l'activité professionel, on ne peut pas tirer de conclusions pouvant influencer l'IMC, les autres facteurs mesurés semble avoir plus d'importance que l'activité professionel. Les raisons de se méfier des valeurs sont que les groupes obèse et le groupe sous-poids ont environ les mêmes proportions alors que l'IMC est très différents entre les 2. 
Les raisons de ces incohérences pourrez être dû aux autres valeurs, les individus ayant une vie sédentaire vont la compenser en fesant plus de sport.
La séparation entre les différents groupes n'est également pas précis. A partir de quel moment, on peut considéré qu'un individu fait une activité professionnel intermédiaire?

Voilà les analyses qui ont été réalisé pendant ce rapport, il y a évidemment plusieurs éléments qui peuvent être améliorer et plusieurs erreurs qui ont été faites lors de l'acquiesement des données. Notre jeu de données aurez pu également être mieux organisé et plus ciblé sur notre étude basé sur l'IMC.

Il y a quelques erreurs au niveau de notre échantillonage la première est le nombre d'individus qui n'est pas assez important et qui peut donc fausser les moyennes.A cause de ce petit nombre d'individus la catégorie sous-poids est très peu représenter et donc une valeur extrêmes peut très vite tout fausser. Nous étions 40 personnes environ à récolter chaqu'un 7 résultats ce qui fait dans les meilleurs cas 280 individus ( dans les meilleurs cas car il peut y avoir des valeurs "NA").
Une autre erreur au niveau de l'échantillonage est qu'il n'est pas assez aléatoire. Il y a par exemple une proportion de jeune beaucoup plus supérieur à la proportion des personnes agées.
L'autre erreur qui a été commis lors de la prise de données, c'est la précision. Pour le grignotage par exemple il n'y a pas été déterminé le type de grignotage : fruits ou friandises. Pour les protéines animales, il aurait peut-être été bien de différencier si il s'agissait de poisson , de volaille ou de viande bovin car ces différentes viandes n'ont pas le même effet sur le poids.Egalement pour l'activité professionnel, il y a une erreur d'imprécision. Les différents groupements n'étant pas clairement défini il est difficile de faire des analyses pertinentes sur cette variable.

Les éléments qui nous pas été abordés dans ce rapport et qui serait intéressant à analyser c'est par exemple les conséquences de l'augmentation de IMC en fonction de groupe formés. Par exemple pour le genre, les femmes ne prennent pas du poids de la même façon que les hommes. Autre élèments important qui serait intéressants à analyser c'est d'autre facteur pouvant influencer un IMC comme par exemple la génétique qui n'a pas été abordé. Le point faible de cette analyse est aussi que l'on a pris chaque facteurs séparément alors qu'il interfère tous entre eux comme par exemple la catégorie normal qui prennaient une trop grande quantité de fastfood mais le compensaient par le sport. Autre chose qui serait intéressant à mesurer ça serait des facteurs lier à la taille car nous avons eessentiellement regarder au poids des individus. Mais la taille aussi influence l'IMC.
On a remarqué les facteurs avaient tous plus ou moins d'influence sur l'IMC, donc l'idée qui pourrait être proposé est d'essayer de numéroter chaque facteur d'une échelle de 1 à 5. Puis additionner toute les valeurs que notre échelle à mesurer pour chaque facteur et ensuite comparer la somme obtenue avec l'IMC de l'individu. 

# Conclusion

En conclusion, l'IMC dépends de l'age et du genre de l'individu à cause de raisons naturel (croissance) ou psycologique ( régime surtout chez les filles). Ensuite on a également beaucoup de facteurs qui influencent les principaux sont l'alcool,les portéines animals, les fastfood et le sport.
Malheureusment notre étude contient beaucoup d'imperfection qui est important de rectifier dans les études à venir. 
