...
chart(data = biometry2, grignotage ~ sequences %fill=% sequences) + geom_boxplot(show.legend = FALSE)+ stat_summary(fun.data = give_n, geom = "text", hjust = 0.5)
...
give_n <- function(x)
  c(y = max(x) * 1.1, label = length(x)) 
chart(data = biometry2, proteine_animale ~ sequences %fill=% sequences) + geom_boxplot(show.legend = FALSE)+ stat_summary(fun.data = give_n, geom = "text", hjust = 0.5)
...
give_n <- function(x)
  c(y = max(x) * 1.1, label = length(x)) 
chart(data = biometry2, fastfood ~ sequences %fill=% sequences) + geom_boxplot(show.legend = FALSE)+ stat_summary(fun.data = give_n, geom = "text", hjust = 0.5)
...
give_n <- function(x)
  c(y = max(x) * 1.1, label = length(x)) 
chart(data = biometry2, acti_physique ~ sequences %fill=% sequences) + geom_boxplot(show.legend = FALSE)+ stat_summary(fun.data = give_n, geom = "text", hjust = 0.5)
...
```{r}
a <- filter(biometry2, sequences == "souspoid")%>.%
...
b <- filter(biometry2, sequences == "normal")%>.%
  chart(., ~ factor(0) %fill=% acti_profession) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
...
c <- filter(biometry2, sequences == "surpoid")%>.%
  chart(., ~ factor(0) %fill=% acti_profession) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
...
d <- filter(biometry2, sequences == "souspoid")%>.%
  chart(., ~ factor(0) %fill=% acti_profession) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
...
### Conséquences de l'augmentation de l'IMC

#### Conséquences en fonction du genre

#### Conséquences en fonction de l'age
