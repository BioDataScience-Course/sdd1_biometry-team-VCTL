...
##on peut se demander si la valeur de notre IMC diffère en fonction de notre appartenance ou non au hainaut.

```{r}
biometry_IMC <- mutate(biometry, IMC = masse/taille^2)
biometry2 <- mutate(biometry_IMC, anormal = IMC > 25 & IMC < 18.5 ,normal = IMC <25 & IMC >= 18.5)
...
```{r}
.mu <-71.7756;.s <-18.3653 ; .mu +.s *qt(0.025, df = 248, lower.tail = FALSE)
.mu <-71.7756;.s <-18.3653 ; .mu +.s *qt(0.025, df = 248, lower.tail = TRUE)
.mu <-71.7756;.s <-18.3653 ; .mu +.s *qt(5.601e-09
...
.mu <- 71.7756; .s <- 15.18; .df <- 248 # .mu, .s (sigma) and .df
...
curve(.d(x), xlim = c(-50,150), xaxs = "i", n = 1000, col = .col,
  add = .add, xlab = "Quantiles", ylab = "Probability density" ) # Curve
abline(h = 0, col = "gray") # Baseline
abline(v = 107.94, col = "red")
abline(v = 35.60, col = "red")
abline(v = -36.77793, col = "green")
```
