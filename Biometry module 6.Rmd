...
biometry <- mutate(biometry, surpoid = IMC >= 25 & IMC < 30, obese = IMC> 30, normal = IMC <25 & IMC >= 18.5 , souspoid = IMC < 18.5)

biometry <- gather(biometry,
   souspoid, normal, obese, surpoid, key = "sequences", value = "rapport_imc")

biometry <- filter (biometry, rapport_imc != FALSE)
