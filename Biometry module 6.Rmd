...
```{r}
biometry_IMC <- mutate(biometry, IMC = masse_corr/taille^2)
...
## On peut se demander si la quantitée de personne classé pour un IMC "normal", diffère en fonction de notre classe d'age.
...
H0 :la probabilité d avoir un IMC de valeur normal pour la population de 20 à 40 ans est la meme que pour la population de 40 à 60 ans

H1 : la probabilité d avoir un IMC qualifié de noraml entre ces 2 groupes diffère

```{r}
biometry %>.%
group_by(., age_rec , rapport_imc)%>.%
biometry<- summarise (., n = n() )%>.%
biometry<- spread(., key = "rapport_imc" , value="n")%>.%
filter(., rapport_imc == "normal" )%>.%
filter (., age_rec !="(0,20]" , age_rec !="(60,110]") 
...
```{r}
chisq.test(biometry, p = c(1/2, 1/2), rescale.p = FALSE)
```
